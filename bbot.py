...
ADMIN_ID = 2054354957
INITIAL_LICENSE_KEY = "n5MM78"

# License key rotation settings
LICENSE_ROTATION_MINUTES = 300
LAST_KEY_CHANGE = time.time()
...
COOLDOWN_SECONDS = 10
FREE_SEARCHES = 10  # Only for new users
WARNING_LIMIT = 5
TEMP_BAN_SECONDS = 600

# Data file in same directory as python file
...
default_data = {
    ...
    "verified_users": {}  # Track which users verified with which key
}
...
def save_data(d=None, reason="auto"):
    """
    Auto-save data with reason tracking
    """
    global data
    if d is None:
        d = data
    ...
    try:
        with data_lock:
            ...
            # Show current credits for active users
            active_users = {k: v for k, v in d['user_searches'].items() if v > 0}
            if active_users:
                print(f"📊 Active credits: {active_users}")
...
def rotate_license_key():
    """Rotate the license key and notify admin"""
    global CURRENT_LICENSE_KEY, LAST_KEY_CHANGE
    ...
    # Notify admin
    try:
        bot.send_message(ADMIN_ID, f"🔄 License Key Rotated!\n\nOld Key: {old_key}\nNew Key: {new_key}\n\nThis key will expire in {LICENSE_ROTATION_MINUTES} minutes.")
        print(f"✅ License key rotated from {old_key} to {new_key}")
    except Exception as e:
        print(f"⚠️ Could not notify admin about key rotation: {e}")

def check_license_rotation():
    """Check if it's time to rotate the license key"""
    global LAST_KEY_CHANGE
    current_time = time.time()
    if current_time - LAST_KEY_CHANGE >= LICENSE_ROTATION_MINUTES * 60:
        rotate_license_key()

def is_user_verified(user_id):
    """Check if user is verified with current license key"""
    uid = str(user_id)
    return data["verified_users"].get(uid) == CURRENT_LICENSE_KEY

def verify_user(user_id):
    """Mark user as verified with current license key"""
    uid = str(user_id)
...
def ensure_user(user_id):
    uid = str(user_id)
    changed = False
    # Only set FREE_SEARCHES if user is completely new (not in data)
...
@auto_save(reason="clear_warnings")
...
def get_remaining_searches(user_id):
    # Always get from saved data, never reset to FREE_SEARCHES
...
def cmd_start(msg):
    chat_id = msg.chat.id
    ensure_user(chat_id)  # This preserves existing credits
    
    # Check license rotation first
    check_license_rotation()
    
    # Check if user needs to re-verify
    if not is_user_verified(chat_id):
        data["user_states"][str(chat_id)] = "awaiting_license"
        save_data(reason="start_state")
...
def handle_message(m):
    ...
    # Check license rotation for every message
    check_license_rotation()
    ...
    if text.startswith("/") and chat_id == ADMIN_ID:
        ...
        if cmd == "/add":
            try:
                ...
                # FIXED: ADD to current credits, don't replace
        ...
        if cmd == "/unban":
            ...
            return
        if cmd == "/newkey":
            # Force rotate license key
            rotate_license_key()
            bot.send_message(ADMIN_ID, f"✅ License key manually rotated to: {CURRENT_LICENSE_KEY}")
            return
        if cmd == "/currentkey":
            time_remaining = LICENSE_ROTATION_MINUTES * 60 - (time.time() - LAST_KEY_CHANGE)
    ...
    if text == "/start":
        ...
        bot.send_message(chat_id, f"🔁 Session started. You have {current_credits} searches.", reply_markup=main_keyboard())
    ...
    if cur_state in ["mobile", "aadhaar", "vehicle", "upi"]:
        ...
        if current_credits <= 0:
            ...
            bot.send_message(chat_id, "💰 Buy Here: https://t.me/PB_I3")
            return
...
def periodic_save():
    """
    Backup periodic save every 30 seconds
    """
    while True:
        time.sleep(30)
...
def license_rotation_monitor():
    """
    Monitor and rotate license keys periodically
    """
    while True:
...
t2 = threading.Thread(target=license_rotation_monitor, daemon=True)
t2.start()
...
def start_bot_with_retry():
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"🔑 Initial License Key: {CURRENT_LICENSE_KEY}")
            print(f"🔄 Key rotation: Every {LICENSE_ROTATION_MINUTES} minutes")
            
            # Test connection first
            test_conn = requests.get("https://api.telegram.org", timeout=10)
            print("✅ Connected to Telegram API")
            
            # Start bot without restart_on_change option
...
print("🤖 Bot starting with dynamic license key system...")
print(f"📁 Data file location: {DATA_FILE}")
print(f"📂 Current directory: {current_dir}")
print("💫 Auto-save system: ACTIVE (real-time updates)")
print("🔄 License rotation: ACTIVE")
